...
impl QuickActionBar {
        ...
        let dropdown_menu = PopoverMenu::new(element_id("menu"))
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    .separator()
                    .action("세션 보기", Box::new(repl::Sessions))
    ...
    pub fn render_repl_launch_menu(
        ...
        let tooltip: SharedString =
            SharedString::from(format!("{} REPL 시작", kernel_specification.name()));
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip: SharedString = SharedString::from(format!("{} REPL 설정", language));
        Some(
            h_flex()
