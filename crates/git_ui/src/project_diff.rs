...
impl Item for ProjectDiff {
    ...
    fn tab_tooltip_text(&self, cx: &App) -> Option<SharedString> {
        match self.diff_base(cx) {
            DiffBase::Head => Some("프로젝트 차이".into()),
            DiffBase::Merge { .. } => Some("브랜치 차이".into()),
        }
    }
    ...
    fn tab_content_text(&self, _detail: usize, cx: &App) -> SharedString {
        match self.branch_diff.read(cx).diff_base() {
            DiffBase::Head => "커밋되지 않은 변경 사항".into(),
            DiffBase::Merge { base_ref } => format!("{} 이후 변경 사항", base_ref).into(),
        }
    }
                ...
                el.child(
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                .justify_around()
                                .child(Label::new("커밋되지 않은 변경 사항 없음")),
                        )
                        .map(|el| match remote_button {
                            ...
                            None => el.child(
                                h_flex()
                                    .justify_around()
                                    .child(Label::new("원격이 최신 상태입니다")),
                            ),
                        })
                        .child(
                            h_flex().justify_around().mt_1().child(
                                Button::new("project-diff-close-button", "닫기")
            ...
            .child(
                h_group_sm()
                    .when(
                        ...
                        |el| {
                            el.child(
                                Button::new("unstage-all", "모두 스테이지 취소")
                                    .tooltip(Tooltip::for_action_title_in(
                                        "모든 변경 사항 스테이지 취소",
                                        &UnstageAll,
                                        &focus_handle,
...
fn render_send_review_to_agent_button(review_count: usize, focus_handle: &FocusHandle) -> Button {
    Button::new(
        "send-review",
        format!("에이전트에 리뷰 보내기 ({})", review_count),
    )
    .start_icon(
...
impl Render for BranchDiffToolbar {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let base_ref_label = format!("기준: {base_ref}");
